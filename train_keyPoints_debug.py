...
from utils.keyPointsLoss import ComputeLoss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_dataloader(val_path, imgsz, batch_size * 2, gs, opt,  # testloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            pbar = tqdm(pbar, total=nb, bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]')  # progress bar
        optimizer.zero_grad()
...
def parse_opt(known=False):
    ...
    opt.device = 0
    opt.batch = 1
    opt.data = 'data/padel_key_points.yaml'
